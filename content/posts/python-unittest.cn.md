...
`unittest` 是 Python 内置的测试框架，不需要安装。它参考了 Java 的 JUnit，测试写在继承 `unittest.TestCase` 的类里。
...
每个测试类继承 `unittest.TestCase`，每个测试方法必须以 `test_` 开头：
...
`unittest` 提供了一套专属的断言方法，不用裸写 `assert`：
...
pytest 可以自动发现并运行 `unittest` 风格的测试，两者完全兼容。新项目基本都用 pytest，`unittest` 在老代码库和标准库里更常见。
