...
列表推导式比 `for` 循环 + `.append()` 更简洁，性能也更好。

---

## 字典

字典存储键值对。键必须唯一且不可变（字符串、数字、元组都可以）。
...
键可能不存在时用 `.get()` 而不是 `[]`——`[]` 会抛出 `KeyError`，`.get()` 返回 `None` 或指定默认值。
...
两者都是有序序列，但列表可变（可以增删改元素），元组不可变（创建后不能修改）。数据不需要改变时用元组，比如坐标或函数返回值。元组速度稍快，还可以作为字典的键。
...
`dict[key]` 在键不存在时抛出 `KeyError`。`dict.get(key)` 默认返回 `None`，也可以指定默认值：`dict.get(key, default)`。生产代码里，除非键不存在代表真正的 bug，否则优先用 `.get()`。
