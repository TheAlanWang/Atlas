...
For simple transformations, list comprehensions are often cleaner and often faster than a `for` loop with `.append()`. Once the logic gets complex, a regular loop is easier to read.
...
Use `.get()` when a missing key is expected and you want a fallback. Use `[]` when a missing key should be treated as a real bug.
...
Both are ordered sequences, but lists are mutable (you can add, remove, or change items) while tuples are immutable (fixed after creation). Use tuples for data that should not change, like coordinates or function return values. Tuples can be used as dictionary keys only when all of their contents are hashable.
...
`dict[key]` raises a `KeyError` if the key does not exist. `dict.get(key)` returns `None` by default, or a custom fallback. Use `.get()` when missing keys are part of normal control flow; use `[]` when absence should fail loudly.
