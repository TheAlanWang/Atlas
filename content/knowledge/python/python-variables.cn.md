...
在 Python 里，变量是通过赋值创建的，不需要先声明类型：
...
Python 是**动态类型**语言——类型附着在运行时的值上，不在变量名上。同一个名字可以在不同时候指向不同类型的对象：
...
用 `type()` 查看一个值的精确类型，用 `isinstance()` 判断它是否符合你预期的类型：
...
Python 支持可选的类型注解。注解不会在运行时强制类型，但能帮助编辑器和静态分析工具发现问题，也让代码更易读：
...
age: int = 25
scores: list[int] = [90, 85, 92]   # 现代内置泛型写法
```

在更老的 typing 写法里，你也会看到来自 `typing` 模块的 `List[int]`。

在面试代码里，类型注解通常不是必须的；在生产代码里，它越来越常见，因为它能让意图更清楚、工具也更好用。

## 可变 vs 不可变

这个区别在传参、复制数据、以及多处共享同一份对象时都很重要：
