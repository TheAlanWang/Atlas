...
测试的作用，是把修改从“猜它没坏”变成“有证据证明它没坏”。

pytest 是一个第三方测试库，也是很多现代 Python 项目的事实默认选择。它写起来简洁，读起来清晰，大型项目也能用。
...
在 pytest 里，测试通常就是一个以 `test_` 开头的函数：
...
`assert` 是测试的核心：表达式为 `False`，测试就失败。

## 项目结构

常见做法是把测试放在单独的 `tests/` 目录里：
...
Fixture 用来提供可复用的测试准备逻辑。`@pytest.fixture` 是 pytest 自带的装饰器，装了 pytest 就能用，不需要额外安装：
...
测试函数的参数名和 fixture 函数名一致时，pytest 会自动注入。注入机制由 pytest 提供，fixture 本身（比如 `sample_list`）仍然是你自己写的代码。
...
把内置 fixture 名字写进参数里，剩下的交给 pytest。
...
这样能避免为同一种断言模式重复写四个测试函数。

## 运行测试

### 自动发现

在项目根目录跑 `pytest` 时，它通常会根据命名约定和项目配置自动发现测试：
